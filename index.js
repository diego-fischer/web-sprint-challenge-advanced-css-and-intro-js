...
function get20s(arr) {
  arr.forEach((element) => {
    let arrYears = [];
    arrYears = element.years.split(" - ");
    element.born = Number(arrYears[0]);
    element.died = Number(arrYears[1]);
    ...
    if (
      ...
      element.twentyfirstcentury = true;
    } else {
      element.twentyfirstcentury = false;
    }
  });
...
const newData = {
  ...
  bio:
    "Diego Fischer is the founder and CEO of CARUPI (https://carupi.com/), which is a P2P marketplace for used cars, and has has previously built the largest C2B car marketplace in Brazil, leading it to US$100M GMV run rate in 2.5yrs, raising US$30M, leading 220 people and conducting a secondary exit. McKinsey, Goldman Sachs and MIT alum (GPA 5.0/5.0). Electrical Engineering undergrad from Polytechnic School of the University of São Paulo.",
};
